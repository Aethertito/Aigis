...
const UserHomeScreen = ({ navigation }) => {
  ...
  const fetchTemperatureSensors = async () => {
    try {
      const response = await axios.get(`http://${IP}:3000/sensor/usuario/${userId}`);
      if (response.data && response.data.length > 0) {
        const tempSensors = response.data.filter(sensor => sensor.tipo === 'Temperature and Humidity');
        setTemperatureSensors(tempSensors);
        setSelectedSensor(tempSensors.length > 0 ? tempSensors[0]._id : null);
      } else {
  ...
  const fetchData = async (mode) => {
    ...
    try {
      ...
      const response = await axios.get(`http://${IP}:3000/api/statistics?sensorId=${selectedSensor}&startDate=${startDate}&endDate=${endDate}`);
    ...
    >
      ...
      <View style={styles.sensorSelector}>
        ...
        <RNPickerSelect
          ...
          items={temperatureSensors.map(sensor => ({
            label: sensor.ubicacion || 'Unspecified location',
            value: sensor._id,
          }))}
